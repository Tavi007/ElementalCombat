...
public class AbsorbParticleData implements IParticleData {

	private Color tint;
	private double diameter;


	public AbsorbParticleData(Color tint, double diameter) {
		this.tint = tint;
	...
	@Nonnull
	@Override
	public ParticleType<AbsorbParticleData> getType() {
		return StartupCommon.absorbParticleType;
	}
	...
	public static final IDeserializer<AbsorbParticleData> DESERIALIZER = new IDeserializer<AbsorbParticleData>() {

		// parse the parameters for this particle from a /particle command
		@Nonnull
		@Override
		public AbsorbParticleData deserialize(@Nonnull ParticleType<AbsorbParticleData> type, @Nonnull StringReader reader) throws CommandSyntaxException {
			reader.expect(' ');
			...
			return new AbsorbParticleData(color, diameter);
		}

		// read the particle information from a PacketBuffer after the client has received it from the server
		@Override
		public AbsorbParticleData read(@Nonnull ParticleType<AbsorbParticleData> type, PacketBuffer buf) {
